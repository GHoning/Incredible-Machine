...
func _fixed_process(delta):
	...
	if overValidSocket && !mouseDown && !attached:
		attached = true
		print("attach")#now tell the parents partent to attach to the current body ???
		get_parent().get_parent().get_parent().set_socket_object1(storeObject)
